...
export const useTrips = (filters?: TripFilters) => {
  ...
  const create = useCallback((payload: Partial<TripData>) => dispatch(createTripAction(payload)), [dispatch]);
  const update = useCallback((id: string, payload: Partial<TripData>) => dispatch(updateTripAction({ id, payload })), [dispatch]);
